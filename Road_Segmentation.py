...
def main():
    ...
    #Allowing the user to resize the images for better visualzation
    # display_width, display_height = 640, 480

    # # Resize images for display
    # resized_background = cv2.resize(img_background, (display_width, display_height))
    # resized_main = cv2.resize(img_main, (display_width, display_height))
    ...
    #Use canny function to detect the edges of the road
    #edges_road = Canny(img_morf) 

    #Extract the lines from the edges
    #lines = cv2.HoughLinesP(edges_road, rho=2, theta=np.pi/180, threshold=100, minLineLength=20, maxLineGap=3)

    #Overlay the road lines on the background image
    #road = draw_lines(resized_background, lines)
    #result = cv2.addWeighted(resized_background, 0.8, road, 1, 1)
